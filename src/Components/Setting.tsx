...
//api
import { Categories } from "../Service/QuizService";
//mui
import {
  InputLabel,
  FormControl,
...
import { SettingFormerrors } from "../Types/ErrorType";
...
const Settings: React.FC<settingPropsType> = ({
  ...
  const submithanddle = (e: React.FormEvent<EventTarget>) => {
    ...
    if (setting.UserName === "") {
      ...
      setError({
        ...error,
        numberOfQues: String("Number of questions should be between 4 to 50"),
      });
    } else {
      setSendRequest(true);
    }
